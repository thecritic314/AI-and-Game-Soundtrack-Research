...
for x in range(1, 19):
    ...
    sheet2 = sheet2 + "(" + str(x) + ")"
    # Load csv files
    df1 = pd.read_excel('data (4).xlsx', sheet_name=sheet1)
    df2 = pd.read_excel('data (4).xlsx', sheet_name=sheet2)
    ...
    # Merge two files
    input_columns_list = ['Timestamp(ms)', '#Balls', '#Bullets', 'Score', 'AvgBtnFreq', 'AvgBallDist', 'RatioBalls',
                          'AvgBallDir']
    ...
    size1 = df1.shape[0]
    size2 = df2.shape[0]
    merge_columns = ['Timestamp(ms)', '#Balls', '#Bullets', 'Score', 'AvgBtnFreq', 'AvgBallDist', 'RatioBalls',
                     'AvgBallDir', 'Intensity', 'Winning Status']
    df_merge = pd.DataFrame(columns=merge_columns)
    # Create initial merge state by merging first row of each
    ts1 = df1['Timestamp(ms)'][0]
    ...
    while i1 < size1 or i2 < size2:
        ts1 = df1['Timestamp(ms)'][min(i1, size1 - 1)]
...
inputs = data.iloc[:, [1, 2, 3, 4, 5, 6, 7]]
