...
'''
This code was written in about 8 hours for the 6th Alakajam,
so a ton of it is rushed and horribly written. :/
'''
...
import csv
import queue
from data_man_3 import model1 as regInt, model2 as regWin, normal_input_scaled, inputs
...
pygame.init()
pygame.display.set_caption('akj game')
WINDOWWIDTH = 500
WINDOWHEIGHT = 700
...
# Dynamic Soundtrack -------------------------
pygame.mixer.music.load('data/music/peaceful.mp3')
global mood, mood_change
mood = 'peaceful'
# change_countdown
peaceful = True
intense = False
...
balls = [[125, 175, 0, 0]]
for i in range(20):
    particles.append(e.particle(balls[-1][0] + random.randint(0, ball_radius * 2) - ball_radius,
                                balls[-1][1] + random.randint(0, ball_radius * 2) - ball_radius, 'p',
                                [random.randint(0, 60) / 10 - 3, random.randint(0, 60) / 10 - 3], 0.2,
                                random.randint(0, 20) / 10, gray))
...
with open("play.csv", "w", newline='') as csvfile:
    writer = csv.writer(csvfile, delimiter='\t')
    writer.writerow(
        ["Timestamp(ms)", "#Balls", "#Bullets", "Score", "AvgBtnFreq", "AvgBallDist", "RatioBalls", "AvgBallDir"])
# play = open("C:\\Users\\Veysel Guney Yilmaz\\Desktop\\Bouncy Shots\\Bouncy Shots Source\\data",'w')
# Loop ------------------------------------------------------- #
...
while True:
    ...
    if instructions == 0:
        ...
        if nearest_ball[0] != None:
            ...
            if opponent_true_cooldown <= 0:
                ...
                projectiles.append(['opponent', opponent_center.copy(), -(opponent_rotation + 13.5),
                                    projectile_data['opponent'][1]])
                opponent_target_offset = random.randint(0, int(opponent_accuracy)) - int(opponent_accuracy / 2)
                shoot_s.play()
    ...
    for ball in balls:
        ...
        if ball[1] < -ball_radius:
            ...
            for ball in balls:
                for i in range(20):
                    particles.append(e.particle(ball[0] + random.randint(0, ball_radius * 2) - ball_radius,
                                                ball[1] + random.randint(0, ball_radius * 2) - ball_radius, 'p',
                                                [random.randint(0, 60) / 10 - 3, random.randint(0, 60) / 10 - 3],
                                                0.2, random.randint(0, 20) / 10, gray))
    ...
    player_r = pygame.Rect(center[0] - polygon_radius * mult, center[1] - polygon_radius * mult,
                           polygon_radius * mult * 2, polygon_radius * mult * 2)

    for i in range(points_in_polygons):
        rot = player_polygon_rot + (i * (360 / points_in_polygons))
        player_points.append([center[0] + math.cos(math.radians(rot)) * polygon_radius,
                              center[1] + math.sin(math.radians(rot)) * polygon_radius])
    pygame.draw.polygon(display, blue, player_points)

    pygame.draw.arc(display, blue, player_r, math.radians(player_rotation + 5), math.radians(player_rotation + 25),
                    max(1, int((player_cooldown / player_max_cooldown) * 5)))
    # opponent
    opponent_cooldown -= 1
    ...
    opponent_r = pygame.Rect(opponent_center[0] - polygon_radius * mult, opponent_center[1] - polygon_radius * mult,
                             polygon_radius * mult * 2, polygon_radius * mult * 2)

    for i in range(points_in_polygons):
        rot = opponent_polygon_rot + (i * (360 / points_in_polygons))
        opponent_points.append([opponent_center[0] + math.cos(math.radians(rot)) * polygon_radius,
                                opponent_center[1] + math.sin(math.radians(rot)) * polygon_radius])
    pygame.draw.polygon(display, red, opponent_points)

    pygame.draw.arc(display, red, opponent_r, math.radians(opponent_rotation + 5),
                    math.radians(opponent_rotation + 25),
                    max(1, int((opponent_cooldown / opponent_max_cooldown) * 5)))
    # Projectiles -------------------------------------------- #
    remove_list = []
    n = 0
    for projectile in projectiles:
        ...
        for i in range(projectile_accuracy):
            projectile[1][0] += core_motion_x * (
                    projectile[3] / projectile_data[projectile[0]][1]) * mult / projectile_accuracy
            projectile[1][1] += core_motion_y * (
                    projectile[3] / projectile_data[projectile[0]][1]) * mult / projectile_accuracy
            for ball in balls:
                ...
                if dis < ball_radius + projectile[3]:
                    ...
                    for i in range(random.randint(2, 4)):
                        slash_particles.append(
                            [advance(projectile[1], math.degrees(collision_angle), projectile[3]),
                             math.degrees(collision_angle) + random.randint(0, 120) - 60, random.randint(32, 72)])
                    for i in range(random.randint(1, 2)):
                        slash_particles.append(
                            [advance(projectile[1], math.degrees(collision_angle), projectile[3]),
                             math.degrees(-collision_angle) + random.randint(0, 120) - 60, random.randint(24, 48)])
                    ball[2] = ball[2] / 3
                    ball[3] = ball[3] / 3
                    ball[2] += math.cos(collision_angle) * projectile_data[projectile[0]][2] * (
                            projectile[3] / projectile_data[projectile[0]][1])
                    ball[3] += math.sin(collision_angle) * projectile_data[projectile[0]][2] * (
                            projectile[3] / projectile_data[projectile[0]][1])
                    remove = True
                    for i in range(int(projectile[3] / 2)):
                        particles.append(
                            e.particle(projectile[1][0] + random.randint(0, int(projectile[3])) - projectile[3] / 2,
                                       projectile[1][1] + random.randint(0, int(projectile[3])) - projectile[3] / 2,
                                       'p', [core_motion_x * 3 + random.randint(0, 40) / 10 - 2,
                                             core_motion_y * 3 + random.randint(0, 40) / 10 - 2], 0.1,
                                       random.randint(0, 30) / 10, colors[projectile[0]]))
        pygame.draw.circle(display, colors[projectile[0]], [int(projectile[1][0]), int(projectile[1][1])],
                           int(projectile[3] + 1))
        projectile[3] -= 0.3 * projectile_data['player'][0] / 10
        particles.append(e.particle(projectile[1][0] + random.randint(0, int(projectile[3])) - projectile[3] / 2,
                                    projectile[1][1] + random.randint(0, int(projectile[3])) - projectile[3] / 2,
                                    'p', [0, 0], 0.2, 0, gray))
        if projectile[3] <= 0:
            remove = True
    ...
    for circle in circle_effects:  # center, radius, time_left
        pygame.draw.circle(display, gray, [int(circle[0][0]), int(circle[0][1])], int(circle[1]),
                           min(int(circle[2] / 3) + 1, int(circle[1])))
        ...
        if event.type == KEYDOWN:
            ...
            if event.key == K_UP:
                ...
                if player_cooldown <= 0:
                    ...
                    projectiles.append(['player', center, -(player_rotation + 13.5),
                                        projectile_data['player'][1]])  # color, position, rotation, size
                    shoot_s.play()
                instructions = 0
        if event.type == KEYUP:
            if event.key == K_RIGHT:
    ...
    if timestamp - past_timestamp >= 100:
        ...
        avg_dir_balls = 0
        for ball in balls:
            avg_dist_balls += display.get_height() / 2 - ball[1]
            if ball[1] <= display.get_height() / 2:
                ratio_balls += 1
            avg_dir_balls += ball[3]
        if len(balls) > 0:
            ratio_balls = ratio_balls / len(balls)
            avg_dir_balls = avg_dir_balls / len(balls)
            avg_dist_balls = avg_dist_balls / len(balls)
        else:
            ratio_balls = 0
            avg_dir_balls = 0
        if key_press_queue.qsize() >= 5:
            key_press_queue.get()
        key_press_queue.put(key_press_count)
        key_press_count = 0
        avg_button_freq = 0
        for elem in tuple(key_press_queue.queue):
            avg_button_freq += elem
        with open("play.csv", "a", newline='') as csvfile:
            writer = csv.writer(csvfile, delimiter='\t')
            if len(balls) != 0:
                curr_inputs = [len(balls),
                          len(projectiles),
                          score,
                          avg_button_freq,
                          round(avg_dist_balls),
                          ratio_balls,
                          "{:.5f}".format(avg_dir_balls)]
                writer.writerow([timestamp] + curr_inputs)
        ...
        # intensity = regInt.predict([[len(balls), len(projectiles), score, avg_button_freq, round(avg_dist_balls),
        #                              ratio_balls, avg_dir_balls]])[0]
        # winning = regWin.predict([[len(balls), len(projectiles), score, avg_button_freq, round(avg_dist_balls),
        #                            ratio_balls, avg_dir_balls]])[0]

        intensity = regInt.predict([curr_inputs])[0]
        winning = regWin.predict([curr_inputs])[0]
        # print(timestamp)
    ...
    if (warping != -1) and (failure == -1):
        if warping > 20:
            ...
            if scored:
                ...
                for i in range(20):
                    particles.append(e.particle(balls[-1][0] + random.randint(0, ball_radius * 2) - ball_radius,
                                                balls[-1][1] + random.randint(0, ball_radius * 2) - ball_radius,
                                                'p',
                                                [random.randint(0, 60) / 10 - 3, random.randint(0, 60) / 10 - 3],
                                                0.2, random.randint(0, 20) / 10, gray))
                circle_effects.append([[125, 175], 10, 20])
        if scored == False:
            if warping > 10:
                warping = -2
        screen.blit(
            pygame.transform.scale(screen, (WINDOWWIDTH + warping * 2, int(WINDOWHEIGHT + warping * 2 * (7 / 5)))),
            (-warping + random.randint(0, 12) - 6, -warping * (7 / 5)))
                ...
                if event.type == KEYDOWN:
                    ...
                    if event.key in [K_RIGHT, K_LEFT, K_UP]:
                        if failure - 30 > score:
                            ...
                            opponent_accuracy = 50
                            opponent_speed = 70
                            opponent_target_offset = random.randint(0, int(opponent_accuracy)) - int(
                                opponent_accuracy / 2)
                            projectile_data = {'player': [8, 20, 1.3], 'opponent': [8, 20, 1.3]}
                            projectiles = []
                            slash_particles = []
                            ...
                            for i in range(20):
                                particles.append(
                                    e.particle(balls[-1][0] + random.randint(0, ball_radius * 2) - ball_radius,
                                               balls[-1][1] + random.randint(0, ball_radius * 2) - ball_radius, 'p',
                                               [random.randint(0, 60) / 10 - 3, random.randint(0, 60) / 10 - 3],
                                               0.2, random.randint(0, 20) / 10, gray))
                            circle_effects.append([[125, 175], 10, 20])
